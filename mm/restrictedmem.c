				 ...
				 u32 request_mask, unsigned int query_flags)
{
	...
	return memfd->f_inode->i_op->getattr(idmap, &memfd->f_path, stat,
			  ...
			  struct restrictedmem_notifier *notifier)
{
	...
	/*
	 * Before returning to the caller to release the 'bindings', wait some
	 * time to allow existing readers to leave.
	 */
	synchronize_rcu();
	up_write(&rm->lock);
}
