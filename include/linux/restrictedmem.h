...
struct restrictedmem_notifier_ops {
	/*
	 * invalidate_start() and invalidate_end() must be paired and the same
	 * range is passed to the two callbacks. invalidate_start() is called
	 * when all pages in the range are still valid and invalidate_end() is
	 * called when all pages in the range have been freed. 'start' is
	 * inclusive and 'end' is exclusive. Please note the range passed to
	 * these callbacks can be larger than the range the notifier originally
	 * bound to, the callbacks are responsible for the range validation.
	 */
