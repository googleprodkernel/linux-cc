...
Errors:
  ...
  ==========  ==================================================
  ENXIO       Flags not supported on current system
  EOPNOTSUPP  Attributes not unsupported on current system
  E2BIG       Address range outside guest physical address space
  EINVAL      Zero size or address/size not page aligned
  ==========  ==================================================
...
by address/size, and in return KVM adjusts address/size to reflect the range of
pages that had its attributes successfully set. If the call returns 0, "address"
is updated to the last successful address + 1 and "size" is updated to the
remaining address size that has not been set successfully. The user should check
the return value as well as the size to decide if the operation succeeded for
the whole range or not. The user may want to retry the operation with the
returned address/size if the previous range was partially successful.
